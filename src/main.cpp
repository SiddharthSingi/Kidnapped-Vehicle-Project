...
int main()
{
  ...
  double sigma_pos [3] = {0.09, 0.09, 0.01}; //earlier {0.3, 0.3, 0.01} GPS measurement uncertainty [x [m], y [m], theta [rad]]
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (!pf.initialized()) {
			...
			if (counter == 906)
			{
				cout << "previous velocity: " << previous_velocity << endl;
				cout << "previous yawrate: " << previous_yawrate << endl;
			}
			*/
		  ...
		  // sense_observations in JSON format [{obs_x,obs_y},{obs_x,obs_y},...{obs_x,obs_y}]
			...
			if (counter == 906)
			{	
				for (int k = 0; k < noisy_observations.size(); k++)
				{
					cout << noisy_observations[k].x << " , " << noisy_observations[k].y << endl;
				}
		  ...
		  if (counter == 906)
		  {
			  ...
			  for (int s = 0; s < 100; s++)
			  {
				  if (pf.particles[s].weight < min_value)
				  {
					  min_value = pf.particles[s].weight;
				  }
			  }
			  cout << "min values = " << min_value << endl;
		  }
		  */
