	...
	*/
	...
	for (int i = 0; i < num_particles; i++)
	{
		...
		for (int k = 0; k < obs_len; k++)
		{
			...
			double x_transformed = observation_vector[k].x * cos(particles[i].theta)
				- observation_vector[k].y * sin(particles[i].theta) + particles[i].x;

			double y_transformed = observation_vector[k].x * sin(particles[i].theta)
				+ observation_vector[k].y * cos(particles[i].theta) + particles[i].y;
		...
		*/
			...
			for (int k = 0; k < nearby_landmarks.size(); k++)
			{
				if (nearby_landmarks[k].id == obs_id)
				{
					...
					cout <<"	obs: (" << observations[j].x << ","<< observations[j].y <<
						") obs_id:" << obs_id << " transformed:(" << transformed_obs[obs_id].x << "," << 
						transformed_obs[obs_id].y << ") lm:(" << nearby_landmarks[k].x 
		...
		//Debugging: cout << "	Total particle weight: " << particles[i].weight << endl;
